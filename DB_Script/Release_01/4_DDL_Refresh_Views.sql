CREATE OR REPLACE FUNCTION analytics.callallmaterializedviews() RETURNS void AS $$  
DECLARE
totalCount integer;
 
    begin
  
PERFORM  analytics.callallmaterializedviewsondemand();

--function for daily backup

totalCount := (SELECT audit."save_intransit_to_fsg_details_into_history_table"());
totalCount := (SELECT audit."save_stock_at_fsg_details_into_history_table"());
totalCount := (SELECT audit."backup_customer_demand_shortage_details_into_history_table"());

--Refresh audit views
REFRESH MATERIALIZED VIEW audit.stock_at_fsg_history_table_view; 
REFRESH MATERIALIZED VIEW audit.intransit_to_fsg_history_table_view;
REFRESH MATERIALIZED VIEW audit.customer_demand_shortage_history_table_view;

REFRESH MATERIALIZED VIEW audit.demand_supply_inventory_graph_view;
REFRESH MATERIALIZED VIEW audit.demand_supply_in_transit_graph_view;
REFRESH MATERIALIZED VIEW audit.demand_supply_demand_graph_view;
REFRESH MATERIALIZED VIEW audit.demand_supply_shortage_graph_view;

REFRESH MATERIALIZED VIEW audit.demand_supply_inventory_drill_down_view;
REFRESH MATERIALIZED VIEW audit.demand_supply_intransit_drill_down_view;
REFRESH MATERIALIZED VIEW audit.demand_supply_demand_shortage_drill_down_view;
...
CREATE FUNCTION analytics.heatmaplandingpageview() RETURNS void AS $$  
BEGIN
REFRESH MATERIALIZED VIEW analytics.heatmap_landing_page_details_view;
 END;
    $$ LANGUAGE plpgsql;

CREATE FUNCTION analytics.heatmapfsgaddressdetailsview() RETURNS void AS $$   
BEGIN
